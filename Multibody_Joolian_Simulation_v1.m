...
simulation_timestep = linspace(0, 20000, 20001);
...
past_positions = cell(1, 6);
planetMarkers = gobjects(1, 6);  % Store handles for planet markers
planetLabels = gobjects(1, 6);   % Store handles for planet labels

for t = 1:100:length(simulation_timestep)
    % Delete previous markers and labels
    delete(planetMarkers);
    delete(planetLabels);

    for i = 1:6
        % Plot trajectory (remove the first 100 positions if more than 5000 are plotted)
        if t <= 5000
            past_positions{i} = [past_positions{i}; planet_positions{i}(t:t+99, :)];
        else
            past_positions{i} = planet_positions{i}(t:t+99, :);
        end

        plot3(past_positions{i}(:, 1), past_positions{i}(:, 2), past_positions{i}(:, 3), 'LineWidth', 1, 'Color', planet_colors(i, :));

        % Plot current position with a circle
        planetMarkers(i) = plot3(planet_positions{i}(t+99, 1), planet_positions{i}(t+99, 2), planet_positions{i}(t+99, 3), 'o', 'MarkerSize', 5, 'Color', planet_colors(i, :), 'MarkerFaceColor', planet_colors(i, :));

        % Plot text labels
        planetLabels(i) = text(planet_positions{i}(t+99, 1), planet_positions{i}(t+99, 2), planet_positions{i}(t+99, 3), planet_names{i},'Color', planet_colors(i, :), 'FontSize', 8, 'HorizontalAlignment', 'left', 'VerticalAlignment', 'bottom');
    end

    % Customize the plot
    axis([-2e8 2e8 -2e8 2e8 -2e8 2e8]);
