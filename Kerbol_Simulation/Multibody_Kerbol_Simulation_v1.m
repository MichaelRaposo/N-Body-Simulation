...
body_prop = cell(1,17);                                 % Preallocation
orbits = cell(1, 17);                                   % Preallocation
r_P = cell(1, 17);                                      % Preallocation
v_P = cell(1, 17);                                      % Preallocation
planet_position = cell(1, 17);                          % Preallocation
planet_velocity = cell(1, 17);                          % Preallocation
planetMarkers = gobjects(1, length(body_prop));         % Store handles for planet markers
planetLabels = gobjects(1, length(body_prop));          % Store handles for planet labels
planetTrajectories = gobjects(1, length(body_prop));    % Store handles for planet trajectories
planetMarkers2 = gobjects(1, length(body_prop));        % Store handles for planet markers
planetLabels2 = gobjects(1, length(body_prop));         % Store handles for planet labels
planetTrajectories2 = gobjects(1, length(body_prop));   % Store handles for planet trajectories
...
% Simulation
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
sim_time = linspace(0, 20000, 20001); % Simulation length in seconds
...
for t = 1:length(sim_time) % Simulation Loop
    ...
    for i = 1:length(orbits) % Current Body
        tot_force_vect = [0, 0, 0]; % Initialize total force vector for each body

        for j = 1:length(orbits) % Interacting Body
        ...
        a_i = tot_force_vect / body_prop{i}(1, 2);
...
for t = 1:length(sim_time)
    if mod(t, spf) == 0
        ...
        for i = 1:length(body_prop) % Planets
            if orbits{i}(1,1) == 1
